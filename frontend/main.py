...
RESOURCE = os.environ.get(
    ...
    "projects/898315975450/locations/us-central1/reasoningEngines/8017484308358889472",
)
...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        storage_client = storage.Client(project="qwiklabs-gcp-02-3932e9061bcb")
        bucket = storage_client.bucket(BUCKET_NAME)
        gcs_path = f"uploads/{uuid.uuid4().hex[:6]}_{file.filename}"
        ...
        return JSONResponse({"status": "success", "filename": file.filename, "public_url": public_url})
...
async def chat(req: Request):
    ...
    if project_name and not message.startswith("[Project Scope:"):
        message = f"[Project Scope: {project_name}] {message}"
